...
namespace Betsson.OnlineWallets.UnitTests.Services
{
    public class OnlineWalletServiceTests
    {
        ...
        [Fact]
        public async Task Withdraw_MoreThanWalletHas_ThrowsErrorAndSavesNothing()
        {
            _repositoryMock
                .Setup(r => r.GetLastOnlineWalletEntryAsync())
                .ReturnsAsync(new OnlineWalletEntry { BalanceBefore = 60m, Amount = 30m });

            await Assert.ThrowsAsync<Exceptions.InsufficientBalanceException>(() =>
                _service.WithdrawFundsAsync(new Models.Withdrawal { Amount = 100m }));

            _repositoryMock.Verify(r => r.InsertOnlineWalletEntryAsync(It.IsAny<OnlineWalletEntry>()), Times.Never);
        }

        [Fact]
        public async Task Withdraw_ExactlyAllTheMoney_AllowsItAndReturnsZeroBalance()
        {
            _repositoryMock
                .Setup(r => r.GetLastOnlineWalletEntryAsync())
                .ReturnsAsync(new OnlineWalletEntry { BalanceBefore = 10m, Amount = 30m });
            _repositoryMock
                .Setup(r => r.InsertOnlineWalletEntryAsync(It.IsAny<OnlineWalletEntry>()))
                .Returns(Task.CompletedTask);

            var balance = await _service.WithdrawFundsAsync(new Models.Withdrawal { Amount = 40m });
        ...
        [Fact]
        public async Task Withdraw_WalletIsEmptyNoHistory_ThrowsErrorEvenForSmallAmount()
        {
            _repositoryMock
                .Setup(r => r.GetLastOnlineWalletEntryAsync())
                .ReturnsAsync((OnlineWalletEntry?)null);
            ...
            await Assert.ThrowsAsync<Exceptions.InsufficientBalanceException>(() =>
                _service.WithdrawFundsAsync(new Models.Withdrawal { Amount = 1m }));

            _repositoryMock.Verify(r => r.InsertOnlineWalletEntryAsync(It.IsAny<OnlineWalletEntry>()), Times.Never);
        }

        [Fact]
        public async Task Withdraw_ZeroAmount_ProcessesNormallyAndBalanceStaysTheSame()
        {
            OnlineWalletEntry? savedEntry = null;
            _repositoryMock
                .Setup(r => r.GetLastOnlineWalletEntryAsync())
            ...
            var balance = await _service.WithdrawFundsAsync(new Models.Withdrawal { Amount = 0m });
